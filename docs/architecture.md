...
**映射优先级**：
1. **系统级配置**: 控制跨平台格式转换的整体策略
2. **调度器级选择**: 根据系统配置选择合适的目标账户
3. **账户级适配**: 同平台内的模型能力适配
...
统一调度器是系统的核心组件，负责管理所有平台的账户选择和负载均衡：
