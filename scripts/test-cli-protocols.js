...
function buildDefaultConfigFromEnv() {
  ...
  if (direct) {
    scenarios.claudeDirect.push(direct)
  }
  ...
  if (bridge) {
    scenarios.claudeBridge.push(bridge)
  }
  ...
  if (codex) {
    scenarios.codexOpenAI.push(codex)
  }
  ...
  if (codexClaude) {
    scenarios.codexClaude.push(codexClaude)
  }
...
function loadConfig() {
  ...
  const hasScenario = Object.values(fallback.scenarios).some(
...
`)
  ...
  if (allResults.length === 0) {
    console.warn('⚠️  No scenarios defined. Nothing to test.')
    return
  }
  ...
  console.log('=== CLI Protocol Test Summary ===')
...
❌ ${failed.length} scenario(s) failed. See table above for details.`)
    ...
    console.log('✅ All configured scenarios completed successfully.')
  }
}

main().catch((err) => {
  console.error('❌ Unexpected error while running CLI protocol tests:', err)
  process.exit(1)
})
