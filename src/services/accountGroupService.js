...
class AccountGroupService {
  ...
  async createGroup(groupData) {
    try {
      ...
      // 验证平台类型
      if (!['claude', 'gemini', 'openai', 'openai-chat', 'droid'].includes(platform)) {
        throw new Error('平台类型必须是 claude、gemini、openai、openai-chat 或 droid')
      }
  ...
  async removeAccountFromAllGroups(accountId, platform = null) {
    try {
      ...
      if (platform) {
        ...
        // 如果没有指定平台，清理所有可能的平台
        const platforms = ['claude', 'gemini', 'openai', 'openai-chat', 'droid']
        const pipeline = client.pipeline()
