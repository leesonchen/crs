/**
 * Claude 中继服务 - 负责与 Claude API 的直接通信和真实使用统计
 *
 * 核心职责：
 * 1. 纯转发功能：构建 HTTP 请求 + 发送到上游 Claude API
 * 2. 真实使用统计：从上游响应中捕获准确的 token 使用量和费用信息
 * 3. 账户状态管理：处理限流、认证失败、过载等账户状态
 * 4. 流式响应处理：支持 SSE 流式转发和使用数据捕获
 * 5. 错误处理：网络错误、API 错误的优雅处理和状态同步
 *
 * 设计原则：
